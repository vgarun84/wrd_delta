...
class FormValidation {
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
       ...
       */
		...
		if (!empty($rules))
		{
			foreach ($rules as $i_rule_index => $rule)
			{
				$rule_params = $this->_rules_params($rule);

				$str_rule  = $rule_params;
				$str_param = null;
				if (is_array($rule_params))
				{
					$str_rule = $rule_params[1];
					$str_param = $rule_params[2];
				}

				$message = $row['errors'][$i_rule_index];
				$restult = $this->$str_rule($postdata, $str_param);
	 ...
	 * @return mixed
	 */
	protected function _rules_params($rule)
	{
		preg_match('/(.*?)\[(.*)\]/', $rule, $match);

		return (empty($match)) ? $rule : $match;
	}
	 ...
	 * @return	bool
	 */
	public function required($str, $dummy = '')
	{
